// Crabcheck fault-localization runner for smallvec. Self-contained.
...
// ---------- helper generators (from src/bin/etna.rs) ----------
...
fn gen_small_vec_u16<R: Rng>(rng: &mut R) -> Vec<u16> {
    // Short Vec range so capacity-0 sources are sampled often (the
    // FromVecZeroCapacity bug only triggers in that case).
...
// ---------- Mutate (single-point perturbations) ----------
...
impl<R: Rng> Mutate<R> for AppendInput {
    fn mutate(&self, rng: &mut R, _n: usize) -> Self {
        ...
        if rng.random_bool(0.5) {
            out.a = mutate_vec_i64(&out.a, rng);
        } else {
            out.b = mutate_vec_i64(&out.b, rng);
        }
        out
    }
}
...
impl<R: Rng> Mutate<R> for FromVecZeroInput {
    fn mutate(&self, rng: &mut R, _n: usize) -> Self {
        ...
        match rng.random_range(0u8..4) {
            ...
            _ => {
                // nudge the reserve hint — the (data.is_empty() &&
                // reserve == 0) corner is the bug-trigger, so mutating
                // here makes it easy to step into / out of that state.
                out.reserve = rng.random::<u8>() % 4;
            },
        }
        out
    }
...
fn main() {
    ...
    if args.len() < 3 {
        ...
        eprintln!("  property: LeakInlinePanics | AppendPreservesLength | FromVecZeroCapacity");
        return;
    }
    ...
    let result = match (tool, property) {
        ("crabcheck", "LeakInlinePanics") => {
            quickcheck(|i: LeakInput| {
                to_opt(property_leak_inline_panics(i.values, i.inline_cap))
            })
        },
        ("crabcheck", "AppendPreservesLength") => {
            quickcheck(|i: AppendInput| {
                to_opt(property_append_preserves_length(i.a, i.b))
            })
        },
        ("crabcheck", "FromVecZeroCapacity") => {
            quickcheck(|i: FromVecZeroInput| {
